...
class MonitorOpinionDistribution(object):
    def __init__(
        self,
        llm_config: dict,
        config_list: dict,
    ...
    def iteration(self, theme: str) -> object:
        ...
        for n1, agent_1 in tqdm.tqdm(list(self.agents.agents_iter())):
            ...
            try:
                new_status, new_status_opponent, text, discussant_text, text2 = (
                    self.debate(agent_1, agent_2, theme)
                )
                ...
                print(f"Error: {e}, {agent_1.name}, {agent_2.name}")
                continue
    ...
    def debate(self, discussant: Agent, opponent: Agent, theme: str) -> (int, str):
        ...
        # add info on neighbors opinions distribution
        neighbors = discussant.get_neighbors()
        opinion_distribution = {k: 0 for k in self.args["opinion_map"].values()}
        for n in neighbors:
            opinion_distribution[
                self.args["opinion_map"][str(self.statuses[n.name])]
            ] += 1
        # express in percentages
        opinion_distribution = {
            k: v / len(neighbors) for k, v in opinion_distribution.items()
        }
        opinion_distribution = "\n".join(
            [f"{k}: {v:.1%}" for k, v in opinion_distribution.items()]
        )
